#!/usr/bin/env python3
"""
Manual test of compiler fingerprinting without full pipeline.
"""
...
def main():
    """Test compiler fingerprinting directly."""
    logging.basicConfig(level=logging.INFO)
    
    print("=== Manual Compiler Fingerprinting Test ===")
    
    # Load synthetic corpus
    corpus_file = Path("./test_clustering_synthetic/corpus.pkl")
    if not corpus_file.exists():
        print("Error: Synthetic corpus not found")
        return 1
    ...
    output_dir = Path("./test_manual_compiler")
    ...
    for binary_name, analysis in per_binary.items():
        print(f"{binary_name}:")
        print(f"  Compiler: {analysis.get('predicted_compiler', 'unknown')} (conf: {analysis.get('compiler_confidence', 0):.3f})")
        print(f"  Optimization: {analysis.get('predicted_optimization', 'unknown')} (conf: {analysis.get('optimization_confidence', 0):.3f})")
    ...
    print(f"\nResults saved to: {output_dir / 'compiler_fingerprinting.json'}")
