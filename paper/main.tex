...
Binary DNA is structured as a modular Python pipeline with a unified CLI
entry point (\texttt{binary\_dna.py}).  Figure~\ref{fig:arch} illustrates
the data flow; Table~\ref{tab:modules} lists the primary modules.

\begin{figure}[h]
\centering
\small
\begin{tabular}{ccccc}
\texttt{/usr/bin/*} & $\rightarrow$ &
\textbf{Extraction} & $\rightarrow$ &
\textbf{Analysis} \\
& & \textit{objdump wrapper} & & \textit{freq, entropy, compress,} \\
& & \textit{corpus manager} & & \textit{motif, clone, LM} \\
& & & $\searrow$ & \\
& & & & \textbf{Clustering} \\
& & & & \textit{NCD, n-gram cosine} \\
& & & & \textit{hierarchical, UMAP} \\
& & $\downarrow$ & & $\downarrow$ \\
\multicolumn{5}{c}{\textbf{Results} (JSON) $\rightarrow$ \textbf{Plots} (PNG) $\rightarrow$ \textbf{HTML report}}
\end{tabular}
\caption{Binary DNA data flow.  Each stage writes structured JSON that the
next stage reads, enabling partial reruns and custom downstream analyses.}
\label{fig:arch}
\end{figure}

\begin{table}[h]
\centering
\caption{Binary DNA modules and their analytical roles.}
\label{tab:modules}
\small
\begin{tabular}{@{}ll@{}}
\toprule
...
\midrule
\texttt{extraction/disassemble.py} & \texttt{objdump} wrapper; mnemonic extraction \\
\texttt{analysis/frequency.py}     & Zipf OLS + MLE, bootstrap CI, KS test \\
\texttt{analysis/ngrams.py}        & $n$-gram entropy, shuffled baseline \\
\texttt{analysis/compression.py}   & \texttt{zlib}/\texttt{lzma} ratios, LZ78 complexity \\
\texttt{analysis/motifs.py}        & $k$-mer motif discovery + annotation \\
\texttt{analysis/information.py}   & MI decay, PCA/MLE intrinsic dimensionality \\
\texttt{analysis/lm.py}            & $n$-gram LM training and perplexity eval \\
\texttt{clustering/ncd.py}         & Normalised compression distance matrix \\
\texttt{clustering/similarity.py}  & $n$-gram TF cosine similarity \\
\texttt{clustering/hierarchical.py}& Agglomerative clustering, cophenetic $r$ \\
\texttt{clones/}                   & MinHash + LSH + Smith-Waterman pipeline \\
\texttt{visualization/report.py}   & Self-contained HTML report generator \\
\bottomrule
\end{tabular}
\end{table}

The CLI exposes subcommands for each stage (\texttt{extract}, \texttt{analyze},
\texttt{lm}, \texttt{clones}, \texttt{compiler-matrix}, \texttt{report}),
allowing users to run the full pipeline or individual analyses independently.
All intermediate results are written as JSON, making them compatible with
downstream tooling (pandas, R, Julia) without using Binary DNA as a dependency.
The toolkit is pure Python ($\geq$3.10) with standard scientific dependencies
(\texttt{numpy}, \texttt{scipy}, \texttt{scikit-learn}, \texttt{seaborn});
no binary-analysis-specific libraries are required beyond \texttt{objdump}
(GNU Binutils), which is pre-installed on Linux.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Corpus} & \textbf{Binaries} & \textbf{Categories} & \textbf{Vocab} & \textbf{Instr.\ (M)} \\
\midrule
Smoke             & 15  & 8   & 293 & 3.12 \\
Coreutils-system  & 39  & 11  & 320 & 4.65 \\
\bottomrule
\end{tabular}
\end{table}
