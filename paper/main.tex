...
find that opcode frequencies follow Zipf's power law with an exponent
$\alpha\!\approx\!3.4$---substantially steeper than English text ($\alpha\!\approx\!1.0$),
reflecting extreme concentration in a handful of opcodes.  N-gram entropy rates
decrease by 1.82 bits/opcode from $n\!=\!1$ to $n\!=\!5$, and the gap between
real and unigram-shuffled baselines confirms genuine higher-order sequential
dependencies.  Binaries compress to roughly $22\%$ of their raw token volume
under \zlib{} compared to $67\%$ for random sequences of the same vocabulary,
evidencing extreme redundancy.  Motif discovery at lengths $k\!=\!4$--$12$
...
\begin{enumerate}
  ...
  \item Empirical evidence that opcode frequencies obey Zipf's law with an
        exponent $\alpha\!\approx\!3.4$, indicating greater concentration
...
$\alpha$ is obtained from 200 resamplings with replacement.
...
\end{enumerate}
Both are compared to a random baseline: sequences of the same length drawn
uniformly from $\mathcal{V}$, repeated ten times and averaged.  Binaries that
are significantly more compressible than random sequences exhibit structural
...
The OLS fit yields $\hat{\alpha} = 3.356$ (95\% CI: $[3.086,\, 3.666]$,
$R^2 = 0.92$) from 200 bootstrap resamples.  This exponent is substantially
larger than the $\alpha \approx 1.0$ characteristic of natural language,
reflecting a far more concentrated opcode distribution: a handful of high-frequency
mnemonics (\texttt{mov}, \texttt{lea}, \texttt{call}, \texttt{push},
\texttt{pop}) account for the vast majority of instructions, while the long
tail of rarely-used opcodes drops off steeply.
...
Table~\ref{tab:compression} summarises compression ratios.  Real binaries
achieve a mean \zlib{} ratio of $0.223$ compared to $0.672$ for random sequences
of the same length and vocabulary---a $67\%$ relative improvement---and $0.185$
vs.\ $0.560$ under \lzma{}.  LZ78 normalised complexity (phrases / $\log_2 N$)
has a median of $630$ for real binaries, versus a raw phrase count of $87{,}881$
for the random baseline, consistent with near-minimal Kolmogorov complexity
for the real instruction streams.
...
\begin{tabular}{@{}lcc@{}}
\toprule
& \textbf{Real} & \textbf{Random baseline} \\
\midrule
\zlib{}       & $0.223$           & $0.672$ \\
\lzma{}       & $0.185$           & $0.560$ \\
LZ78 (norm.)  & $630$ (med.)      & $\gg\!10^4$ \\
\bottomrule
\end{tabular}
\end{table}
...
The combination of a Zipf-distributed vocabulary ($\alpha \approx 3.4$), rapidly
...
disassembly beyond opcode extraction.
...
\begin{itemize}
  \item The effective vocabulary size is $|\mathcal{V}| = 293$
        ...
        of magnitude.
  \item Transition probabilities are highly non-uniform (Zipf, $\alpha \approx
        3.4$), meaning a handful of high-frequency opcodes carry nearly all of
...
million instructions, we demonstrated that opcode frequencies obey Zipf's law
($\alpha \approx 3.4$, steeper than natural language), entropy rates decrease
by $1.82$ bits from $n=1$ to $n=5$ (vs.\ $1.09$ bits for a shuffled baseline,
confirming genuine sequential dependencies), binaries compress to $22\%$ of
raw token volume vs.\ $67\%$ for random sequences of the same vocabulary,
...
predictable instruction sequences are for a language model.
