...
in natural language and biological sequences.  We present \emph{Binary DNA}
...
study of program instruction sequences.

Across a corpus of 15 system utilities totalling 3.1~million instructions, we
find that opcode frequencies exhibit a highly concentrated, heavy-tailed
distribution (MLE exponent $\hat{\alpha}_{\text{MLE}}\!\approx\!1.4$, closer to
English text's $\alpha\!\approx\!1.0$; OLS log-log exponent
$\hat{\alpha}_{\text{OLS}}\!\approx\!3.4$, inflated by long-tail weighting
bias; KS $=0.114$, indicating approximately but not strictly a power law).
...
sequences and $67\%$ for random sequences; the
unigram--sequential decomposition attributes $69\%$ of the compression gain to
frequency skew and $31\%$ to sequential structure.  Motif discovery at lengths
$k\!=\!4$--$12$ recovers 798 recurring patterns dominated by compiler-generated
idioms.  PCA on per-binary bigram frequency vectors reveals that the corpus
manifold occupies only 4\% of the theoretical bigram space
($d_{95}\!=\!8$ out of 200); Levina--Bickel MLE corroborates this at
$\hat{d}\!=\!8.0$.  A bigram language model achieves perplexity $8.5$---a
$34\times$ reduction below vocabulary size---with a smoothing gap of only
$0.07$~bits from the empirical conditional entropy lower bound.
Scaling to 39 binaries (4.65~million instructions) confirms that all
opcode-level statistics are stable across corpus size while $d_{95}$ grows
modestly to 14, reflecting greater categorical diversity.
\end{abstract}
