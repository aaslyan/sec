...
\begin{enumerate}
  ...
  \item \textbf{LZ78 complexity}~\cite{ziv1978compression}: the number of
        ...
        integer-encoded opcode sequence of length $N$, normalised as
        $\tilde{c} = K / \log_2 N$ (dimensionless phrase count per bit of
        sequence length).  A maximally random sequence of length $N$ over
        alphabet $\mathcal{V}$ yields $\tilde{c} \sim N/\log_2 N$; highly
        compressible sequences yield much smaller $\tilde{c}$.
\end{enumerate}
...
Since $K$ scales as $\Theta(N/\log N)$ for high-entropy sources, $\tilde{c}$
is expected to be on the order of $N/\log_2 N$ for a near-uniform source,
which explains the large random-baseline magnitude (to calibrate: a sequence of
$N\!=\!10^6$ tokens would yield $\tilde{c} \approx N/\log_2 N \approx 50{,}000$
at maximum entropy, so $\tilde{c}\!=\!87{,}881$ for our random baseline is
consistent with near-maximum-entropy behaviour while $\tilde{c}\!=\!630$
is roughly $70\times$ below that floor, placing real instruction streams
substantially below both baselines).
...
factor to why decompilers guided by LLMs may recover meaningful variable names
and control structures from assembly in practice.
