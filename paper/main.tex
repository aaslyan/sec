...
Counter-intuitively, \texttt{-O3} increases mean instruction count
($601$ vs.\ $499$ at \texttt{-O0}) while also increasing compressibility
for numeric codes (matrix: $0.375$, the lowest of any configuration)
due to aggressive loop unrolling creating longer repeated blocks; text
and search programs exhibit the opposite trend, remaining near \texttt{-O2}
levels.  \texttt{-O2} consistently produces the \emph{least} compressible
output, suggesting it generates the most diverse (least-repetitive) instruction
mix before \texttt{-O3} re-introduces repetition through unrolling.
